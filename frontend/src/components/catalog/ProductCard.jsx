  ...
  return (
    <Card className="product-card h-100 overflow-hidden">
      <Link to={`/catalog/${product.slug}`} className="product-image-wrap">
        {image ? <CloudinaryImage src={image} alt={product.name} className="product-image" width={480} /> : <div className="product-image product-image-fallback" aria-hidden="true" />}
        {product.discount_percent > 0 && <span className="badge product-badge">-{product.discount_percent}%</span>}
      </Link>
      <div className="p-3">
        <p className="eyebrow mb-1">{product.brand?.name}</p>
