...
export function ShopTheLookPage() {
  ...
  return <section className="container py-5 feature-page"><div className="row g-4"><div className="col-lg-5"><img className="shop-look-image" src="https://images.unsplash.com/photo-1496747611176-843222e1e57c?auto=format&fit=crop&w=1000&q=80" alt="After-hours linen look" /></div><div className="col-lg-7"><p className="eyebrow">SHOP THE LOOK</p><h1>After-hours linen</h1><p className="text-slate-300">A study in relaxed structure from Amara’s summer edit. Pick individual pieces or add the full look.</p>{notice && <div className="ai-server-notice">{notice}</div>}<div className="d-grid gap-2 my-4">{products.map((product) => <div className="look-product velora-card p-2" key={product.id}><img src={product.image} alt="" /><div><p className="eyebrow mb-1">{product.brand}</p><strong>{product.name}</strong><small>${product.price}</small></div><button className="btn btn-velora-secondary" onClick={() => add(product)}>Add</button></div>)}</div><button className="btn btn-velora-primary w-100" onClick={addAll}><FiShoppingBag /> Add complete look · $316</button></div></div></section>
}
...
export function FollowersPage() {
  ...
  return <section className="container py-5 feature-page"><p className="eyebrow">YOUR NETWORK</p><h1>Following</h1><p className="text-slate-300">Keep up with the people whose edits you return to.</p><div className="row g-3 mt-2">{creators.filter((creator) => following.includes(creator.id)).map((creator) => <div className="col-md-6" key={creator.id}><div className="follow-row velora-card p-3"><img src={creator.image} alt="" /><div><strong>{creator.name}</strong><small>{creator.handle} · {creator.followers} followers</small></div><button className="btn btn-velora-secondary ms-auto" onClick={() => toggleFollow(creator.id)}>Following</button></div></div>)}</div>{!following.length && <div className="velora-card p-4 mt-3"><FiUsers /> <strong className="ms-2">Follow creators to build your circle.</strong></div>}</section>
}
