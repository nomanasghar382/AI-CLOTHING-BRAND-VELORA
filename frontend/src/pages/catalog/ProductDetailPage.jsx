...
export default function ProductDetailPage() {
  ...
  return <section className="container py-4 py-lg-5"><Seo title={product.name} description={product.short_description || product.description} image={primary?.url} type="product" jsonLd={jsonLd} /><Breadcrumb items={[{ label: 'Catalog', to: '/catalog' }, { label: product.name }]} /><div className="row g-4"><div className="col-lg-7"><div className="detail-image-wrap">{primary && <CloudinaryImage src={primary.url} alt={product.name} className="detail-image" width={960} />}</div><div className="d-flex gap-2 mt-3">{product.images?.slice(0, 5).map((image, index) => <img key={index} className="detail-thumbnail" src={image.thumbnail_url} alt="" loading="lazy" />)}</div></div><div className="col-lg-5"><p className="eyebrow">{product.brand?.name} / {product.category?.name}</p><h1 className="display-6">{product.name}</h1><div className="my-3"><span className="product-price fs-3">${Number(product.sale_price || product.price).toFixed(2)}</span>{product.sale_price && <del className="ms-2 text-slate-300">${Number(product.price).toFixed(2)}</del>}</div><p className="text-slate-300">{product.description}</p><div className="spec-grid my-4"><span>Fabric <strong>{product.fabric}</strong></span><span>Fit <strong>{product.fit_type}</strong></span><span>Coverage <strong>{product.coverage_level}</strong></span><span>SKU <strong>{product.sku}</strong></span></div><p className="small text-slate-300 mb-2">AVAILABLE COLORS</p><div className="d-flex gap-2 mb-4">{product.colors?.map((color) => <span key={color.name} title={color.name} className="color-swatch" style={{ background: color.hex_code }} />)}</div><p className="small text-slate-300 mb-2">SIZES</p><div className="d-flex gap-2 flex-wrap">{product.sizes?.map((size) => <button className="btn btn-velora-secondary" type="button" key={size.name}>{size.name}</button>)}</div>{actionError && <div className="alert alert-danger mt-3 mb-0" role="alert">{actionError}</div>}<div className="d-flex gap-2 mt-4"><button className="btn btn-velora-primary flex-grow-1" type="button" disabled={busy} onClick={() => perform(() => addToCart(item))}><FiShoppingBag /> Add to bag</button><button className="btn btn-velora-secondary" type="button" disabled={busy} onClick={() => perform(() => addToWishlist(item))} aria-label="Add to wishlist"><FiHeart /></button></div><Link className="btn btn-velora-ghost mt-3" to="/catalog">Continue exploring</Link></div></div><article className="product-passport mt-5"><div><p className="eyebrow">DIGITAL PRODUCT PASSPORT</p><h2><FiFeather /> Made to be known.</h2><p>Trace the materials, care and afterlife of this piece in one transparent record.</p></div><div className="passport-grid"><span>Materials<strong>{product.fabric || 'Details forthcoming'}</strong></span><span>Origin<strong>{product.origin || product.country_of_origin || 'Verified at fulfillment'}</strong></span><span>Care<strong>{product.care_instructions || 'Follow garment label'}</strong></span><span>End of life<strong>{product.recycling_guidance || 'Repair, resell or recycle responsibly'}</strong></span></div><Link className="btn btn-velora-secondary" to="/alerts"><FiAlertCircle /> Set price or restock alert</Link></article></section>
}
