...
final class VisualSearchService
{
    ...
    private function providerResults(UploadedFile $image): array
    {
        ...
        return Product::query()->where('status', 'published')->where('stock_quantity', '>', 0)->with('images')->limit(12)->get()->map(fn (Product $p) => ['product_id' => $p->id, 'score' => 0, 'image_url' => $p->images->first()?->url])->all();
    }
}
